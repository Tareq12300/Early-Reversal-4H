...
def rsi(series, period=14):
    ...
    TradingView يستخدم RMA داخل ta.rsi.
    """
    delta = series.diff()
...
def calculate_multi_score(signals):
    ...
    macd_positive_count = sum(
        1 for s in signals
        if s["macd"] > 0 and s["macd"] > s["macd_prev"]
    )
...
def scanner_loop():
    ...
    while True:
        try:
            ...
            print(
                f"Full scan finished. Candidates: {len(all_signals)} | Multi alerts sent: {multi_sent}"
            )
